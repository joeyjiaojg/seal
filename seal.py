...
def polinfo(args):
    ...
    if args.info_domains:
        ...
        for dom in list(p.domains.keys()):
            print(dom)
    else:
...
def get_file_permissions(policy, files_dict, processes_dict):
    ...
    for f in list(files_dict.values()):
    ...
    if args.out:
        ...
        with open(output, "w") as thefile:
            if files_dict and proc_permissions:
                ...
                print(tmp.format(len(processes_dict)), file=thefile)
                ...
                for fname, procperm_dict in proc_permissions.items():
                    ...
                    print(tmp.format(f.security_class, fname,
                                               f.context, len(procperm_dict)), file=thefile)
